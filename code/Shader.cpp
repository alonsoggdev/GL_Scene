...
namespace udit
{
   ...
   const std::string Shader::default_fragment_shader_code =
    ...
    : m_type(type), m_vertex_source(vertex_source), m_fragment_source(fragment_source), m_name(name)
    {
        ...
        try
        {
            std::string relative_path = std::getenv("CODE_PATH");
            std::ifstream vertex_file(relative_path + vertex_source);
            std::ifstream fragment_file(relative_path + fragment_source);
    ...
    {
        std::string relative_path = std::getenv("RESOURCES_PATH");
        ...
        if (type == udit::ShaderType::TERRAIN)
        {
            ...
            std::string complete_path = relative_path + texture_paths[0];
        ...
        {
            if (path == "") break;
            std::string complete_path = relative_path + path;
